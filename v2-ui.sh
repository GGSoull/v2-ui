...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/zhubb.net/sh/master/tcp.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/gosoull/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
